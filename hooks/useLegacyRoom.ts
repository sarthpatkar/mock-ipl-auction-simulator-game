...
export type LegacyRealtimeStatus = 'connecting' | 'live' | 'degraded' | 'offline'
...
export function useLegacyRoom(optionsRoomId: string | null, options: UseRoomOptions = {}) {
  ...
  const [connectionState, setConnectionState] = useState<LegacyRealtimeStatus>('connecting')
  const hasHydratedRef = useRef(false)
  const priorConnectionRef = useRef<LegacyRealtimeStatus>('connecting')
  ...
  const hydrate = useCallback(async () => {
    ...
    try {
      ...
      const { data: roomData, error: roomError } = await supabaseClient.from('rooms').select(ROOM_SELECT).eq('id', optionsRoomId).maybeSingle()
  ...
  }, [enabled, includeRemoved, optionsRoomId])
  ...
  useEffect(() => {
    if (!enabled || !optionsRoomId) return

    setConnectionState('connecting')

    const channel = supabaseClient
      .channel(`room:${optionsRoomId}`)
      .on(
        'postgres_changes',
        { event: 'UPDATE', schema: 'public', table: 'rooms', filter: `id=eq.${optionsRoomId}` },
        (payload) => {
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'room_participants', filter: `room_id=eq.${optionsRoomId}` },
        async (payload) => {
          try {
  ...
  }, [enabled, fetchParticipantSnapshot, hydrate, includeRemoved, optionsRoomId])
