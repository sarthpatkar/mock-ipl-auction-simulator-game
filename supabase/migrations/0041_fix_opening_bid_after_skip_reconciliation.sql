...
begin
  ...
  -- Only reconcile before the bid when there is already an active highest bidder.
  -- For opening bids, we must let the bid land first so it can reset manual skip
  -- state and reopen the player to previously skipped participants.
